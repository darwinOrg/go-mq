...
func (a *RedisListAdapter) Subscribe(ctx context.Context, topic string, handler SubscribeHandler) error {
	a.RedisCli.BRPop(ctx, topic, a.Timeout, func(value string) error {
		dc := &dgctx.DgContext{TraceId: uuid.NewString()}
		return handler(dc, value)
	})
